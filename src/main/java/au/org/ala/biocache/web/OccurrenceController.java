...
public class OccurrenceController extends AbstractSecureController {
  ...
  @Inject protected IndexDAO indexDao;

  /** Data Resource DAO */
  @Inject protected SearchUtils searchUtils;

  @Inject protected RestfulClient restfulClient;
  @Inject protected SpeciesLookupService speciesLookupService;
  @Inject protected AuthService authService;
  @Inject protected ContactUtils contactUtils;
  @Inject protected OccurrenceUtils occurrenceUtils;
  @Inject protected AssertionUtils assertionUtils;
  @Inject protected DownloadService downloadService;
  @Inject private AbstractMessageSource messageSource;
  @Inject private ImageMetadataService imageMetadataService;
  @Autowired private Validator validator;
  @Inject protected QidCacheDAO qidCacheDao;
  @Inject private CacheManager cacheManager;
  @Inject private LayersService layersService;
  ...
  @Value(
      "${taxon.id.pattern:urn:lsid:biodiversity.org.au[a-zA-Z0-9\\.:-]*|http://id.biodiversity.org.au/[a-zA-Z0-9/]*}")
  ...
  private final AtomicReference<String> occurrenceETag =
      new AtomicReference<String>(UUID.randomUUID().toString());
  ...
  /**
   * Returns the content of the messages.properties file. Can also return language specific
   * versions, such as messages_fr.properties if requested via qualifier @PathVariable.
   *
   * @param response
   * @throws Exception
   */
  @RequestMapping(value = "/facets/i18n{qualifier:.*}*", method = RequestMethod.GET)
  public void writei18nPropertiesFile(
    ...
    if (f.exists() && f.isFile() && f.canRead()) {
      ...
      is =
          request
              .getSession()
              .getServletContext()
              .getResourceAsStream("/WEB-INF/messages" + qualifier);
    }
  ...
  public @ResponseBody Collection<IndexFieldDTO> getIndexedFields(
    ...
    if (indexed != null
      ...
      Set<String> dataTypes =
          dataType == null ? null : new HashSet(Arrays.asList(dataType.split(",")));
  ...
  public void getIndexedFields(
    ...
    Collection<IndexFieldDTO> indexedFields =
        getIndexedFields(fields, indexed, stored, multivalue, dataType, classs, isMisc, isDwc);
   ...
   *
   * <p>Can force the refresh if an apiKey is also provided. e.g. after a known edit.
   *
   * @return
   * @throws Exception
   */
  ...
  /**
   * Returns a list of image urls for the supplied taxon guid. An empty list is returned when no
  ...
  public @ResponseBody List<String> getImages(HttpServletRequest request) throws Exception {
    ...
    if (results.getFacetResults().size() > 0) {
      List<FieldResultDTO> fieldResults =
          results.getFacetResults().iterator().next().getFieldResult();
      ArrayList<String> images = new ArrayList<String>(fieldResults.size());
      for (FieldResultDTO fr : fieldResults) images.add(fr.getLabel());
      return images;
    }
   ...
   */
  @RequestMapping(
      value = {"/australian/taxon/**", "/native/taxon/**"},
  ...
  @RequestMapping(
      value = {
        "/australian/taxa.json*",
        "/australian/taxa*",
        "/native/taxa.json*",
        "/native/taxa*"
      },
      method = RequestMethod.GET)
  public @ResponseBody List<NativeDTO> isAustralianForList(
      @RequestParam(value = "guids", required = true) String guids) throws Exception {
  ...
  /**
   * Service to determine if a GUID has native or isAustralian status TODO should be in a service
   *
   * @param guid
   * @return
   */
   ...
   */
  @RequestMapping(
      value = {"/occurrences/taxon/**", "/occurrences/taxon/**", "/occurrences/taxa/**"},
      method = RequestMethod.GET)
  public @ResponseBody SearchResultDTO occurrenceSearchByTaxon(
      SpatialSearchRequestParams requestParams, HttpServletRequest request) throws Exception {
    String guid = searchUtils.getGuidFromPath(request);
   ...
   *
   * <p>I don't think that this should be necessary. We should be able to configure the
   * requestParams facets to contain the collectino_uid, institution_uid data_resource_uid and
   * data_provider_uid
   *
   * <p>It also handle's the logging for the BIE. //TODO Work out what to do with this
   *
   * @throws Exception
   */
  ...
  public @ResponseBody SearchResultDTO occurrenceSearchForUID(
      SpatialSearchRequestParams requestParams, @PathVariable("uid") String uid) throws Exception {
  ...
  public @ResponseBody SearchResultDTO occurrenceSearchByArea(
      SpatialSearchRequestParams requestParams, Model model) {
    SearchResultDTO searchResult = new SearchResultDTO();
  ...
  private SearchResultDTO occurrenceSearch(SpatialSearchRequestParams requestParams)
      throws Exception {
   ...
   */
  @RequestMapping(
      value = {"/occurrences/search.json*", "/occurrences/search*"},
      method = RequestMethod.GET)
  public @ResponseBody SearchResultDTO occurrenceSearch(
    ...
    Map<String, String[]> map =
        request != null ? SearchUtils.getExtraParams(request.getParameterMap()) : null;
    if (map != null) {
      map.remove("apiKey");
    }
    ...
    if (srtdto.getTotalRecords() > 0 && lookupImageMetadata) {
      ...
      Map<String, List<Map<String, Object>>> imageMap =
          imageMetadataService.getImageMetadataForOccurrences(occurrenceIDs);
  ...
  /**
   * Regenerate the ETag after clearing the cache so that cached responses are identified as out of
   * date
   */
   ...
   *
   * <p>ONLY 1 facet should be included in the params.
   *
   * @param requestParams
   * @param response
   * @throws Exception
  ...
  public void downloadFacet(
    ...
    if (requestParams.getFacets().length > 0) {
      ...
      DownloadDetailsDTO dd =
          downloadService.registerDownload(
              requestParams, ip, DownloadDetailsDTO.DownloadType.FACET);
      try {
        String filename =
            requestParams.getFile() != null
                ? requestParams.getFile()
                : requestParams.getFacets()[0];
        response.setHeader("Cache-Control", "must-revalidate");
        response.setHeader("Pragma", "must-revalidate");
  ...
  /**
   * Webservice to support bulk downloads for a long list of queries for a single field. NOTE:
   * triggered on "Download Records" button
   *
   * @param response
   * @param request
   * @param separator
  ...
  public void batchDownload(
    ...
    if (qid != null) {
      ...
      String webservicesRoot =
          request.getSession().getServletContext().getInitParameter("webservicesRoot");
      response.sendRedirect(
          webservicesRoot + "/occurrences/download?" + downloadRequestParams.getEncodedParams());
    } else {
  ...
  public String batchDownload(
    ...
    final DownloadDetailsDTO dd =
        downloadService.registerDownload(params, ip, DownloadType.RECORDS_INDEX);
                  ...
                  if (lsid != null) {
                    try {
                      ...
                      String outputFilePath =
                          directory + File.separatorChar + row[0].replace(" ", "_") + ".txt";
                      ...
                      try (FileOutputStream output = new FileOutputStream(outputFilePath); ) {
                        ...
                        ConcurrentMap<String, AtomicInteger> uidStats =
                            searchDAO.writeResultsFromIndexToStream(
                                params, new CloseShieldOutputStream(output), false, dd, false);
                        output.flush();
                        try (FileOutputStream citationOutput =
                            new FileOutputStream(citationFilePath); ) {
  ...
  /**
   * Given a list of queries for a single field, return an AJAX response with the qid (cached query
   * id) NOTE: triggered on "Search" button
   *
   * @param response
   * @param queries
   * @param separator
   ...
   */
  private Long getQidForBatchSearch(
      String listOfNames, String field, String separator, String title)
      throws IOException, QidSizeException {
   ...
   * Occurrence search page uses SOLR JSON to display results
   *
   * <p>Please NOTE that the q and fq provided to this URL should be obtained from
   * SearchResultDTO.urlParameters
   *
   * @return
   * @throws Exception
   */
  ...
  public String occurrenceIndexDownload(
      @Valid DownloadRequestParams requestParams,
      BindingResult result,
      @RequestParam(value = "apiKey", required = false) String apiKey,
      @RequestParam(value = "ip", required = false) String ip,
  ...
  /**
   * Returns the IP address for the supplied request. It will look for the existence of an
   * X-Forwarded-For Header before extracting it from the request.
   *
   * @param request
  ...
  /**
   * Utility method for retrieving a list of occurrences. Mainly added to help debug web services
   * for that a developer can retrieve example UUIDs.
   *
   * @throws Exception
   */
  @RequestMapping(
      value = {"/occurrences/nearest"},
      method = RequestMethod.GET)
  public @ResponseBody Map<String, Object> nearestOccurrence(
      SpatialSearchRequestParams requestParams) throws Exception {

    logger.debug(
        String.format(
            "Received lat: %f, lon:%f, radius:%f",
            requestParams.getLat(), requestParams.getLon(), requestParams.getRadius()));
  ...
  /**
   * Dumps the distinct latitudes and longitudes that are used in the connected index (to 4 decimal
   * places)
   */
   ...
   *
   * <p>When user supplies a uuid that is not found search for a unique record with the supplied
   * occurrenc_id
   *
   * <p>Returns a SearchResultDTO when there is more than 1 record with the supplied UUID
   *
   * <p>TODO move to service layer
   *
   * @param uuid
   * @throws Exception
   */
   ...
   */
  @RequestMapping(
      value = {"/occurrence/compare*"},
      method = RequestMethod.GET)
  public @ResponseBody Object compareOccurrenceVersions(
  ...
  @RequestMapping(
      value = {
        ...
        "/senstive/occurrences/{uuid:.+}.json"
      },
      method = RequestMethod.GET)
  ...
  private void logViewEvent(String ip, SolrDocument occ, String email, String reason) {
    ...
    LogEventVO vo =
        new LogEventVO(LogEventType.OCCURRENCE_RECORDS_VIEWED, email, reason, ip, uidStats);
    logger.log(RestLevel.REMOTE, vo);
  }

  /**
   * Constructs an error message to be displayed. The error message is based on validation checks
   * that were performed and stored in the supplied result.
   *
   * <p>TODO: If we decide to perform more detailed validations elsewhere it maybe worth providing
   * this in a util or service class.
   *
  ...
  private String getValidationErrorMessage(BindingResult result) {
    ...
    for (ObjectError error : errors) {
      ...
      sb.append(messageSource.getMessage(code, null, error.getDefaultMessage(), null))
          .append("<br/>");
    }
    return sb.toString();
  }
        ...
        try {
          List<Map<String, Object>> list =
              imageMetadataService
                  .getImageMetadataForOccurrences(Arrays.asList(new String[] {uuid}))
                  .get(uuid);
          if (list != null) {
            for (Map m : list) {
   ...
   *
   * <p>Requires valid apiKey.
   *
   * <p>facets is the pivot facet list
   */
  @RequestMapping("occurrence/pivot")
  public @ResponseBody List<FacetPivotResultDTO> searchPivot(
   ...
   *
   * <p>Requires valid apiKey because it is very slow.
   */
  @RequestMapping("occurrences/facets/available")
  public @ResponseBody List<String> listFacets(
      SpatialSearchRequestParams searchParams,
      @RequestParam(value = "apiKey", required = true) String apiKey,
      HttpServletResponse response)
      throws Exception {
