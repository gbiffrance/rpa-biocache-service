...
public class SolrIndexDAOImpl implements IndexDAO {
  ...
  @Inject protected QueryFormatUtils queryFormatUtils;
  ...
  Map<String, String> legacyToNew = new ConcurrentHashMap<>();
  Map<String, String> newToLegacy = new ConcurrentHashMap<>();
  ...
  public void init() {
    ...
    // Field mapping between SOLR schema version 1.0 to version 2.0
    try {
      InputStream is = this.getClass().getResourceAsStream("/newToLegacyFieldNames.properties");
      Properties properties = new Properties();
      properties.load(is);

      for (Map.Entry<Object, Object> p : properties.entrySet()) {
        legacyToNew.put(p.getKey().toString(), p.getValue().toString());
        newToLegacy.put(p.getValue().toString(), p.getKey().toString());
      }
    } catch (Exception e) {
      logger.error("Failed to load resource \"newToLegacyFieldNames.properties\"");
    }
  }
  ...
  @Override
  public QueryResponse query(SolrParams query, Double version) throws Exception {
    int retry = 0;
    ...
    while (retry < maxRetries && qr == null) {
      ...
      try {
        ...
        if (legacyFieldNameSupport && version == null || version == 1.0) {
          ...
          qr = toLegacyResponse(qr, pair.second());
        } else {
          qr = solrClient.query(query, SolrRequest.METHOD.POST); // can throw exception
        }
  ...
  private QueryResponse toLegacyResponse(QueryResponse qr, Map<String, String> newToLegacy) {
    //        qr.getExpandedResults();
    //        qr.getExplainMap();

    List<FacetField> fd = qr.getFacetDates();
    List<FacetField> ff = qr.getFacetFields();

    // getFacetPivot is handled elsewhere
    // NamedList<List<PivotField>> fp = qr.getFacetPivot();

    Map<String, Integer> fq = qr.getFacetQuery();
    List<RangeFacet> rf = qr.getFacetRanges();

    //        qr.getIntervalFacets();
    //        qr.getClusteringResponse();

    Map<String, FieldStatsInfo> fsi = qr.getFieldStatsInfo();

    //        qr.getGroupResponse();
    //        qr.getHeader();
    //        qr.getHighlighting();
    //        qr.getJsonFacetingResponse();
    //        qr.getLimitingFacets();

    SolrDocumentList results = qr.getResults();

    for (Map.Entry<String, String> es : newToLegacy.entrySet()) {
      for (int i = 0; fd != null && i < fd.size(); i++) {
        if (fd.get(i).getName().equals(es.getKey())) {
          fd.set(i, new FacetFieldRenamed(es.getValue(), fd.get(i)));
        }
      }

      for (int i = 0; ff != null && i < ff.size(); i++) {
        if (ff.get(i).getName().equals(es.getKey())) {
          ff.set(i, new FacetFieldRenamed(es.getValue(), ff.get(i)));
        }
      }

      if (fq != null && fq.containsKey(es.getKey())) {
        fq.put(es.getValue(), fq.get(es.getKey()));
        fq.remove(es.getKey());
      }

      for (int i = 0; rf != null && i < rf.size(); i++) {
        if (rf.get(i).getName().equals(es.getKey())) {
          rf.set(i, new RangeFacetRenamed(es.getValue(), rf.get(i)));
        }
      }

      if (fsi != null && fsi.containsKey(es.getKey())) {
        fsi.put(es.getValue(), fsi.get(es.getKey()));
        fsi.remove(es.getKey());
      }
    }

    if (results != null) {
      // if the first result does not require field translation, do not continue
      boolean replacementRequired = false;
      for (int i = 0; i < results.size() && (replacementRequired || i == 0); i++) {
        SolrDocument sd = results.get(i);

        // assume that newToLegacy is not a large list
        for (Map.Entry<String, String> es : newToLegacy.entrySet()) {
          if (sd.containsKey(es.getKey())) {
            // add with the old field name
            sd.setField(es.getValue(), sd.getFieldValue(es.getKey()));

            // remove the replaced field
            sd.removeFields(es.getKey());
  ...
  private Pair<SolrParams, Map<String, String>> toLegacyQuery(SolrParams query) {
    Map<String, String> newToLegacy = new HashMap();

    for (Iterator<Map.Entry<String, String[]>> it = query.iterator(); it.hasNext(); ) {
      ...
      if (entity.getValue() != null) {
        ...
        for (int i = 0; i < values.length; i++) {
          ...
          if (value != null) {
            ...
            Matcher m;
            String matchEnd;
            if ("q".equals(entity.getKey()) || "fq".equals(entity.getKey())) {
              m = q.matcher(value);
              matchEnd = ":";
            } else {
              m = f.matcher(value);
            ...
            while (m.find()) {
              ...
              String newField = legacyToNew.get(requestedField);
              if (newField != null) {
                newToLegacy.put(newField, requestedField);
                m.appendReplacement(sb, newField + matchEnd);
              }
            }
            m.appendTail(sb);
    ...
    return new Pair(query, newToLegacy);
  }
  ...
  public Set<IndexFieldDTO> getIndexFieldDetails(String... fields) throws Exception {
    ...
    QueryResponse response = query(params, 2.0);
  ...
  @Inject protected AbstractMessageSource messageSource;
  ...
  public QueryResponse runSolrQuery(SolrQuery solrQuery, SearchRequestParams requestParams)
    ...
    QueryResponse qr = query(solrQuery, requestParams.getVersion()); // can throw exception
    if (logger.isDebugEnabled()) {
  ...
  @Override
  public Map<String, String> getNewToLegacy() {
    return newToLegacy;
  }
}
